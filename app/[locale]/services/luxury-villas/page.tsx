...
import ServiceHeroParallax from "@/components/services/ServiceHeroParallax";
...
import ServiceWalkthroughVideo from "@/components/services/ServiceWalkthroughVideo";
...
import { whatsappLuxuryVillasLink, SITE_URL } from "@/lib/constants";
import { routing, type Locale } from "@/i18n/routing";

export const revalidate = 60;

const SERVICE_PATHNAME = "/services/luxury-villas" as const;
const SERVICE = getServiceByPathname(SERVICE_PATHNAME)!;

const WALKTHROUGH_VIDEO = "/luxury-villa-walkthrough.mp4";
const WALKTHROUGH_POSTER = "/portfolio-realestate.jpg";
...
export default async function LuxuryVillasServicePage({ params }: Props) {
  const { locale } = await params;
  ...
  const whatsappHref = whatsappLuxuryVillasLink(locale as "es" | "en");
  ...
  return (
    <>
      ...
      <main id="main-content" style={{ background: "var(--bg)" }}>
        ...
        <ServiceHeroParallax service={SERVICE} whatsappHref={whatsappHref} />
        <ServiceSectionPainPoints service={SERVICE} />
        <ServiceSectionIncludes service={SERVICE} />
        <ServiceWalkthroughVideo
          service={SERVICE}
          videoSrc={WALKTHROUGH_VIDEO}
          posterSrc={WALKTHROUGH_POSTER}
        />
